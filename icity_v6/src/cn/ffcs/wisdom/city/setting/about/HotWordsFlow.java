 ...
 * <p>Title:  自定义热词控件				   </p>
 * <p>Description:<br/>          
 * 		旧版热词显示采用绝对布局，这有一定的局限性，不能很好的适应屏幕，故重新自定义下控件
 * </p>
 * <p>@author: liaodl                  </p>
 ...
 */
public class HotWordsFlow extends FrameLayout implements OnGlobalLayoutListener {
    ...
    public HotWordsFlow(Context context, AttributeSet attrs, int defStyle) {  
        super(context, attrs, defStyle); 
        this.context = context;
        init();  
    }  
  
    public HotWordsFlow(Context context, AttributeSet attrs) {  
        super(context, attrs);  
        this.context = context;
        init();  
    }  
    ...
    private boolean show() {  
        if (width > 0 && height > 0 && vecKeywords != null && vecKeywords.size() > 0 && enableShow) {  
            ...
            int xItem = width / size, yItem = (height-20) / size;
...
//                txt.setBackgroundColor(Color.RED);
    ...
    public void setOnGestureListener(OnGestureListener listener) {  
        ...
        setLongClickable(true);//加上这句才可以正确识别手势，否则只会返回Down, Show, Long三种手势
