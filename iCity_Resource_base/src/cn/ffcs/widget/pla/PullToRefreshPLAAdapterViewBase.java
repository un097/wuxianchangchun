...
import cn.ffcs.widget.pla.internal.PLA_AbsListView.OnScrollListener;


public abstract class PullToRefreshPLAAdapterViewBase<T extends PLA_AbsListView> extends PullToRefreshBase<T> implements
		OnScrollListener {
	...
	public PullToRefreshPLAAdapterViewBase(Context context, int mode) {
		super(context, mode);
		refreshableView.setOnScrollListener(this);
	}

	public PullToRefreshPLAAdapterViewBase(Context context, AttributeSet attrs) {
		super(context, attrs);
		refreshableView.setOnScrollListener(this);
	}
	...
	public final void onScrollStateChanged(final PLA_AbsListView view, final int scrollState) {
		if (null != onScrollListener) {
	...
	public void setBackToTopView(ImageView mTopImageView){
		this.setmTopImageView(mTopImageView);
		mTopImageView.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				if (refreshableView instanceof MultiColumnListView ) {
					((MultiColumnListView)refreshableView).setSelection(0);
				}
			}
		});
	}
	...
	public void setmTopImageView(ImageView mTopImageView) {
