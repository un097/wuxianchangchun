...
public class TrafficApplication extends Application {
	...
	public void initEngineManager(Context context) {
		...
		if (!mBMapManager.init(null)) {
			Toast.makeText(
					TrafficApplication.getInstance().getApplicationContext(),
	...
	public static class MyGeneralListener implements MKGeneralListener {
		...
		public void onGetNetworkState(int iError) {
			if (iError == MKEvent.ERROR_NETWORK_CONNECT) {
				Toast.makeText(
						TrafficApplication.getInstance()
		...
		public void onGetPermissionState(int iError) {
			...
			if (iError != 0) {
				// 授权Key错误：
				// Toast.makeText(TrafficApplication.getInstance().getApplicationContext(),
				// "请在 DemoApplication.java文件输入正确的授权Key,并检查您的网络连接是否正常！error: " +
				// iError,
				// Toast.LENGTH_LONG).show();
				TrafficApplication.getInstance().m_bKeyRight = false;
			} else {
				TrafficApplication.getInstance().m_bKeyRight = true;
				// Toast.makeText(TrafficApplication.getInstance().getApplicationContext(),
				// "key认证成功",
				// Toast.LENGTH_LONG).show();
			}
