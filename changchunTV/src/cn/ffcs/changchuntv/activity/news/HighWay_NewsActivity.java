 ...
 *
 */
public class HighWay_NewsActivity extends BaseFragmentActivity implements OnClickListener, CategoryChangedListener {
	
	private ViewPager mViewPager;
	private HighWay_NewsViewPagerAdapter mAdapter;
	...
	public void onClick(View v) {
		...
		if (id == R.id.icon_category) {
			...
			HighWay_NewsFragment fragment = (HighWay_NewsFragment) mAdapter.getFragment(index);
			if (fragment != null)
				fragment.refresh();
		}
	}
...
//			}
	...
	private void loadChannel() {
		...
		for (int i = 0; i < ids.length; i++) {
			HighWay_NewsFragment fragment = (HighWay_NewsFragment) HighWay_NewsFragment.newInstance(ids[i]);
			fragment.setTitle(list.get(i).mName);
			flist.add(fragment);
		}
		mAdapter = new HighWay_NewsViewPagerAdapter(getSupportFragmentManager(), flist);
		mViewPager.setAdapter(mAdapter);
