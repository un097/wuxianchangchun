...
import cn.ffcs.changchuntv.activity.news.HighWay_NewsAdapter.NewsDelegate;
...
public class HighWay_NewsFragment extends BaseFragment implements OnRefreshListener<ListView>,
		OnLastItemVisibleListener, NewsDelegate {

	private static final String TAG = HighWay_NewsFragment.class.getSimpleName();
	private static final String TAG_INDEX_NEWS = "TAG_INDEX_NEWS";
	private static final String PREFIX_NEWS = "PREFIX_NEWS_";

	private HighWay_NewsAdapter mAdapter;
	...
	private HighWay_NewsActivity mActivity;
	...
	public static Fragment newInstance(int chnl_id) {
		HighWay_NewsFragment f = new HighWay_NewsFragment();
	...
	protected void initview(View view) {
		...
		mAdapter = new HighWay_NewsAdapter(this);
...
//				Log.e("sb", view.getScrollY() + "                            ----------");
	...
	public void onAttach(Activity activity) {
		...
		mActivity = (HighWay_NewsActivity) activity;
	}
...
//				List<Advertising> banner_advs = new ArrayList<AdvertisingEntity.Advertising>();
//				for (int i = 0; i < 5; i++) {
//					Advertising entity = new AdvertisingEntity().new Advertising();
//					entity.group = -1;
//					entity.android_pic = "http://img5.imgtn.bdimg.com/it/u=1041153494,1223721837&fm=116&gp=0.jpg";
//					entity.id = i;
//					entity.order = i;
//					entity.title = "测试数据" + i;
...
//				initBanner(adEntity);
				...
				try {
					...
					adEntity = JsonUtil.toObject(jarray.get(0).toString(), AdvertisingEntity.class);
					if (adEntity.banner_advs.size() != 0) {
						if (headerView == null) {
							headerView = new AdvertisingHeaderView(mActivity, mChnl_id);
	...
	private void getNewsData() {
		...
		if (mPageNumber != 1 || current - t >= s_time || mNeedRefresh) {
			...
			hasLoadedWeb = true;
			if (mChnl_id == 0) {
				GetIndexNewsHome req = new GetIndexNewsHome(mPageNumber, mFirstVisitTime,
						baseRequest, baseError);
				Application.instance().addToRequestQueue(req, TAG_INDEX_NEWS);
			} else {
				GetIndexNewsByChannel req = new GetIndexNewsByChannel(mPageNumber, mChnl_id,
	...
	private void handleNewsJSONArray(JSONArray array) throws Exception {
		...
		ArrayList<NewsGroup> news = News.makeAll(mAdapter.getData(), array, mIds);
	...
	private void handleUpdateCountTip(HashSet<Integer> mIds) {
		if (mPageNumber == 1) { // 如果是刷新这提示更新条数
			final HashSet<Integer> lastIds = Application.getLastNewsIds(mChnl_id);
			...
			if (mFromRefresh) {// 不是第一次刷新，且是下拉刷新
				...
				new Handler().postDelayed(new Runnable() {
					...
					public void run() {
						if (updateCount <= 0)
							mTvUpdateCount.setText(R.string.tip_news_update_none);
						else
							mTvUpdateCount.setText(mActivity.getString(R.string.tip_news_update_count,
									updateCount));
	...
	class LoadNewsCacheTask extends AsyncTask<Void, Void, JSONArray> {
		...
		protected JSONArray doInBackground(Void... params) {
			String cacheContent = IndexCacheUtils.getCache(PREFIX_NEWS + mChnl_id);
			...
			if (result == null) {
				if (hasLoadedWeb) {
					...
					if (mChnl_id == 0) {
						...
						GetIndexNewsByChannel req = new GetIndexNewsByChannel(mPageNumber,
								mChnl_id, mFirstVisitTime, baseRequest, baseError);
						Application.instance().addToRequestQueue(req, TAG_INDEX_NEWS);
					}
				}
			} else {
	...
	public void onNewsItemClick(News news) {
		...
		intent.putExtra("titleString", "高速路况");
		intent.setExtrasClassLoader(News.class.getClassLoader());
		startActivity(intent);
		// 记录日志
		Application.instance().addEventLog("index", String.valueOf(news.getId()), "content");
