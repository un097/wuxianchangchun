...
public class TDevice {
	...
	public static String getIMSI() {
		TelephonyManager mTelephonyMgr = (TelephonyManager) BaseApplication
	...
	public static String getAssertFileString(String fileName) {
		StringBuffer sb = new StringBuffer();
		InputStream is = null;
		try {
			is = BaseApplication.context().getAssets().open(fileName, 2);
			if (is != null) {
				BufferedReader br = new BufferedReader(
						new InputStreamReader(is));
				String temp = null;
				while ((temp = br.readLine()) != null) {
					sb.append((new StringBuilder(temp).append("\r\n")
							.toString()));
				}
			}
		} catch (IOException e) {
	...
	/*修改*///以下变量方法新增
	private static final int PIC_WIDTH[] = new int[] { 250, 280, 320, 480 };

	public static int getNeighbourWidth(float f, boolean flag) {
		int i = PIC_WIDTH[0];
		if (!flag) {
			int j = -1 + PIC_WIDTH.length;
			while (j > -1) {
				if (f < (float) PIC_WIDTH[j]) {
					j--;
				} else {
					i = PIC_WIDTH[j];
					break;
			...
			int k = PIC_WIDTH.length;
			int l = 0;
			while (l < k) {
				if (f > (float) PIC_WIDTH[l]) {
					l++;
				} else {
					i = PIC_WIDTH[l];
					break;
