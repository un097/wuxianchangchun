...
import com.ctbri.wxcc.videoview.R;
...
public class VideoView extends SurfaceView implements
    ...
    public void setVideoLayout(int layout) {
        ...
        if (mVideoHeight > 0 && mVideoWidth > 0) {
            ...
            }else if (VIDEO_LAYOUT_FIT == layout){
            	final int mWidth = getWidth();
            	final int mHeight = getHeight();
            	if(mWidth > 0 && mHeight > 0){
//            		float mRealRatio = mWidth / (float)mHeight;
            		int mFixedWidth = mSurfaceWidth *   mHeight / mVideoHeight;
            		getHolder().setFixedSize(mFixedWidth, mHeight);
            		lp.width = mFixedWidth;
            		lp.height = mHeight;
            	}
