...
public class CommonNewTask extends BaseTask<Void, Void, Void> {
	 ...
	 * @return
	 */
	public static CommonNewTask newInstance(HttpCallBack iCall, Context context, Class<?> toJsonObjectClass) {
		return new CommonNewTask(iCall, context, toJsonObjectClass);
	}
...
//	}
	...
	private CommonNewTask(HttpCallBack iCall, Context context, Class<?> clazz) {
		super(iCall, context);
		this.clazz = clazz;
	}
	
	private CommonNewTask(HttpCallBack iCall, Class<?> clazz) {
		super(iCall);
		this.clazz = clazz;
	}
